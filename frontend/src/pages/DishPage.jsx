...
const ITEM_THUMB = 64;   // صورة مصغّرة
const ITEM_MIN_H = 84;   // ارتفاع صف العنصر
const ACTIONS_W = 64;    // عرض الأزرار الجانبية

// ===== قصّ بسيط بالحروف (لا يعدّل المحتوى، فقط يقتطع البداية) =====
const hardClamp = (value = '', max = 120) => {
  const s = String(value);
  return s.length > max ? s.slice(0, max) + '…' : s;
};
// سطران → يسمح بطول مبدئي قبل القص الحرفي
const NAME_MAX = 25;
const DESC_MAX = 30;
    ...
    >
      ...
      <Grid container spacing={2} alignItems="flex-start">
        ...
        <Grid item xs={12} md={8} sx={{ minWidth: 0 }}>
          ...
          {/* بانر */}
          <Card sx={{ borderRadius: 3, mb: 2, overflow: 'hidden', background: 'linear-gradient(135deg, #08090bff 0%, rgba(8, 8, 18, 1)00%)', color: 'white' }}>
            <CardContent sx={{ py: { xs: 3, md: 4 } }}>
              ...
              <Box sx={{ mt: 2, display: 'inline-flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(255,255,255,.08)', borderRadius: 999, px: 1.5, py: 0.75 }}>
                <Typography variant="caption" sx={{ color: 'white' }}>{t('Contact support') || 'Contact support'}</Typography>
                ...
                {filteredDishes.map((dish) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* المحتوى — نعرض أول سطرين فقط للاسم والوصف */}
                      <Box
                        sx={{
                          minWidth: 0,
                          overflow: 'hidden',
                          ...(isRTL ? { pl: `${ACTIONS_W}px` } : { pr: `${ACTIONS_W}px` }),
                        }}
                      >
                        {/* الاسم — سطران */}
                        <Typography
                        ...
                        <Box
                          sx={{
                            ...
                            overflow: 'hidden',
                            minWidth: 0,
                            maskImage: 'linear-gradient(to left, transparent 0, black 24px, black 100%)',
                          }}
                        >
                          {prices.map((p) => (
                            <Chip
